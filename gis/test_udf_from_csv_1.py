...
if __name__ == "__main__":
    ...
    l = ['10_5', '10_6', '10_7', '10_8', '10_9']
    with open("/home/shengjh/Desktop/test_udf.sh", 'a') as f:
        f.write("#!/bin/bash\n")
        for n in l:
            for test in test_name:
                if test in ['st_envelope_aggr', 'st_union_aggr']:
                    continue
                str = "spark-submit --master yarn --deploy-mode cluster  $HOME/arctern_profile/gis/test_udf_from_csv.py -p hdfs://arctern-master:9000/test_data/%s -o hdfs://arcter:9000/test_report/%s -f %s" % (
                    n, n, test)
                f.write(str + '\n')
            for test in ['st_envelope_aggr', 'st_union_aggr']:
                str = "spark-submit --master yarn --deploy-mode cluster  $HOME/arctern_profile/gis/test_udf_from_csv.py -p hdfs://spark01:9000/test_data/%s -o hdfs://spark01:9000/test_report/%s -f %s" % (
                    n, n, test)
                f.write(str + '\n')
            f.write("echo test　%s done.\n" % n)
